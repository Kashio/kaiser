...
BasicFetcher.prototype.logic = function(resource, callback) {
	...
	if (self.crawler.isStopping) {
		...
		callback(new Error('Fetch failed because of robots txt'));
	}
...
BasicFetcher.prototype.requestLoop = function(options, attemptsLeft, resource, callback, lastError) {
	var self = this;
	if (attemptsLeft <= 0) {
		self.handleRequest((lastError != null ? lastError : new Error('No attempts to fetch the URL were made')), null, null, resource, callback);
	} else {
		request.get(options, function (error, response, body) {
			...
			if ((error && _.contains(recoverableErrors, error.code)) || (response && (500 <= response.statusCode && response.statusCode < 600))) {
				...
				self.handleRequest(null, response, body, resource, callback);
			} else if (error) {
				e = new Error(util.format("Error fetching '%s': %s (%s)", options.uri, error.message, error.code));
				e.code = error.code;
				self.handleRequest(e, null, null, resource, callback);
			} else {
 ...
 * @param {endLogicCallback} callback
 */
BasicFetcher.prototype.handleRequest = function(error, response, body, resource, callback) {
	var self = this;
	if (!error) {

...
BasicTransformer.prototype.calculateReplacePortionOfFetchedUris = function(resource, uri) {
	...
	if (uri === '/') {
		...
		if (resource.uri.hostname() != uriObj.hostname()) {
			calculatedPath = self.calculateReplacePortionOfFetchedUriWithDifferentHostname(resource, uriObj);
		} else {
			calculatedPath = self.calculateReplacePortionOfFetchedUriWithEqualHostname(resource, uriObj);
		}
...
/**
 * Calculate the new relative uri path of a fetched uri of a different hostname
 *
 * @param {Resource} resource
 * @param {URI} uriObj
 * @returns {String}
 */
BasicTransformer.prototype.calculateReplacePortionOfFetchedUriWithDifferentHostname = function(resource, uriObj) {
	...
	return path.join(relativness, uriObj.hostname(), uriObj.directory(), helpers.makeFileNameFromUri(uriObj));
};

/**
 * Calculate the new relative uri path of a fetched uri of an equal hostname
 *
 * @param {Resource} resource
 * @param {URI} uriObj
 * @returns {String}
 */
BasicTransformer.prototype.calculateReplacePortionOfFetchedUriWithEqualHostname = function(resource, uriObj) {
	var newDirPortion = '';
	...
	return path.join(relativness, newDirPortion, helpers.makeFileNameFromUri(uriObj));
};

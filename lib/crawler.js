...
Crawler.init = function(options) {
    ...
    if (!self.cache) {
        if (options.cache && options.cache instanceof Cache.Cache) {
            self.cache = options.cache;
        } else {
            self.cache = new Cache.FsCache({
                rootDir: './cache'
            });
        }
    }
    ...
    if (self.canTransform(resource)) {
        ...
        async.waterfall([
            ...
            function (uris, resource, callback) {
                self.reduceUris.call(self, uris, callback);
            }
        ], function (err, result) {
...
Crawler.prototype.transformStrategy = function(uris, resource) {
    ...
    _.each(uris, function(uri) {
        ...
        var relatedness = (resource.uri.hostname() === uriObj.hostname() ?
            '.' :
            '..');
        var replacement = util.format('"%s\/%s%s"', relatedness, uriObj.host(), uriObj.resource());
        resource.content = replaceAll(resource.content, util.format('"%s"', uri), replacement);
    });
};

...
var helpers            = require('./helpers'),
    ...
    FetchQueue         = require('./fetch_queue/fetch_queue'),
    BasicFetchQueue    = require('./fetch_queue/basic_fetch_queue'),
...
Crawler.init = function(options) {
    ...
    if (!self.fetcher) {
        self.fetcher = options.fetcher && options.fetcher instanceof Fetcher ?
            options.fetcher :
            new BasicFetcher(self, request.defaults({
                proxy: options.proxy ?
    ...
    if (!self.fetchQueue) {
        self.fetchQueue = options.fetchQueue && options.fetchQueue instanceof FetchQueue ?
            options.fetchQueue :
            new BasicFetchQueue();
    }
...
Crawler.prototype.crawl = function(uris, originator) {
    ...
    async.waterfall([
        ...
        self.fetchQueue.queue.bind(self.fetchQueue),
